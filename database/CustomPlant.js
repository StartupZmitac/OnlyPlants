...
const [db, setDb] = useState(SQLite.openDatabase('PlantsDatabase.db'));
    ...
    const addPlant = (currentPlant) => {
        db.transaction(tx => {
          tx.executeSql('INSERT INTO plants (name, default_interval, sunlight, cycle, edible, poisonous, indoor) values (?)', [currentPlant],
            (txObj, resultSet) => {},
    ...
    useEffect(() => {
        db.transaction(tx => {
          ...
          + "default_interval INTEGER, sunlight TEXT, cycle TEXT, edible INTEGER, poisonous INTEGER, indoor INTEGER)")
        });
      }, [db]);
  ...
  return (
    <View style={{padding: 10}}>
      ...
      <TextInput
        ...
        defaultValue={""}
      />
      <Button title="Add Plant" onPress={addPlant([name, interval, sunlight, cycle, edible, poisonous, indoor])}/>
    </View>
  );
};
